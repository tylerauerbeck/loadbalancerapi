		...
		hook.On(
			func(next ent.Mutator) ent.Mutator {
				return hook.OriginFunc(func(ctx context.Context, m *generated.OriginMutation) (ent.Value, error) {
					...
					addSubjPoolTenantID, err := m.Client().Pool.Query().Where(pool.HasOriginWith(origin.IDEQ(objID))).Only(ctx)
					if err == nil {
		...
		hook.On(
			func(next ent.Mutator) ent.Mutator {
				return hook.PoolFunc(func(ctx context.Context, m *generated.PoolMutation) (ent.Value, error) {
					...
					// addSubjPortLoadBalancerID, err := m.Client().Port.Get(ctx, objID)
					addSubjPortLoadBalancerID, err := m.Client().Port.Query().Where(port.HasPoolWith(pool.IDEQ(objID))).Only(ctx)
					if err == nil {
						if !slices.Contains(additionalSubjects, addSubjPortLoadBalancerID.ID) && objID != addSubjPortLoadBalancerID.ID {
							additionalSubjects = append(additionalSubjects, addSubjPortLoadBalancerID.ID)
						}
		...
		hook.On(
			func(next ent.Mutator) ent.Mutator {
				return hook.PortFunc(func(ctx context.Context, m *generated.PortMutation) (ent.Value, error) {
					...
					// addSubjPoolTenantID, err := m.Client().Pool.Get(ctx, objID)
					addSubjPoolTenantID, err := m.Client().Pool.Query().Where(pool.HasPortWith(port.IDEQ(objID))).Only(ctx)
					if err == nil {
						if !slices.Contains(additionalSubjects, addSubjPoolTenantID.ID) && objID != addSubjPoolTenantID.ID {
							additionalSubjects = append(additionalSubjects, addSubjPoolTenantID.ID)
						}
					...
					addSubjLoadBalancerTenantID, err := m.Client().LoadBalancer.Query().Where(loadbalancer.HasPortWith(port.IDEQ(objID))).Only(ctx)
					if err == nil {
					...
					addSubjLoadBalancerLocationID, err := m.Client().LoadBalancer.Query().Where(loadbalancer.HasPortWith(port.IDEQ(objID))).Only(ctx)
					if err == nil {
					...
					addSubjLoadBalancerProviderID, err := m.Client().LoadBalancer.Query().Where(loadbalancer.HasPortWith(port.IDEQ(objID))).Only(ctx)
					if err == nil {
